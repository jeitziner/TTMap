...
".normMesh",sep = ""));
write.pcl(Disease.pcl,paste(org.directory,dataname,
".normMesh",sep = ""));
}
...
U <- lapply(1: length(unique(B)),function(i){intersect(names(B[B[]==(i-1)]),colnames(Normal.pcl))
})
...
plot(E,V,main=paste("Mean/variance/plot",paste("batch",i,sep=""),sep=""), 
...
flat.Nmat <- FLAT.hda2(Normal.mat, 
...
write.table(u,file=paste(paste(paste(org.directory,dataname,sep="/"),
paste("batch",i-1,sep=""),sep=""),"na_numbers_per_row.txt",sep="_"),
quote=FALSE,sep="\t",row.names=TRUE,col.names=TRUE)
    u <- as.matrix(flat.Nmat$m)
   write.table(u,file=paste(paste(paste(org.directory,dataname,sep="/"),
paste("batch",i-1,sep=""),sep=""),"na_numbers_per_col.txt",sep="_"),
quote=FALSE,sep="\t",row.names=TRUE,col.names=TRUE)
