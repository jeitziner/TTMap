...
annot=ttmap_part1$tag.pcl,col = "NAME",
...
filename,sep="/"),
paste(c[ttmap_part3$low_map[[i]][1],n],
"txt",sep="."),sep="_"),
quote=FALSE,sep="\t",row.names=TRUE,col.names=NA)
}
for(i in 1: length(ttmap_part3$mid1_map)){
A <-ttmap_part4_inter(ttmap_part3$mid1_map[[i]],
ttmap_part2,alpha=a)
A<- as.matrix(A)
A <- as.data.frame(A)
...
filename,sep="/"),
paste(c[ttmap_part3$mid1_map[[i]][1],n],
"txt",sep="."),sep="_"),
quote=FALSE,sep="\t",row.names=TRUE,col.names=NA)
}
for(i in 1: length(ttmap_part3$mid2_map)){
A <-ttmap_part4_inter(ttmap_part3$mid2_map[[i]],
ttmap_part2,alpha=a)
A<- as.matrix(A)
A <- as.data.frame(A)
A <-cbind(annot[rownames(A),col],A)
