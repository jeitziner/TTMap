ttmap_part3 <-
function(ttmap_part2,m1,select=row.names(ttmap_part2$Dc.Dmat),ddd,e,filename="TEST",n=3,ad=0,bd=0,piq=1,dd=generate_mismatch_distance(ttmap_part2=ttmap_part2,select=select),mean_value_m1 ="N",ni=2){
	write.table(dd,file="distance_matrix.txt",sep="\t",quote = FALSE, row.names = TRUE,col.names=NA)
	minespace <- 50
	beg <- ls()
...
q_low <- lapply(1:length(q_low), function(i){
 colnames(as.matrix(dd)[names(low),names(low)])[as.vector(q_low[[i]])]
})
}
...
q_high <- lapply( 1:length(q_high),function(i){
	colnames(as.matrix(dd)[names(high),names(high)])[as.vector(q_high[[i]])]
})
}
...
junk <- list(r = r_all,s = size_all,f = f_all,q1_all = q1_all,q1_all_a = q1_all_a)
return(junk)	
}
...
spheres3d(p$f[i],0,0,radius=p$s[i],color=rgb((p$r)$col[[i]][1],(p$r)$col[[i]][2],(p$r)$col[[i]][3],alpha=1))
text3d(p$f[i],max(as.matrix(unlist(p$s))) + 5,0,i)
if(ad==0){text3d(p$f[i],-max(as.matrix(unlist(p$s))) - 5,0,paste(unique(p$q1_all_a[[i]]),collapse=""))}
})


m <-0
if(dim(low_map)[1]==1 && low_map==0){du<-max(as.matrix(unlist(size_all))) + minespace
d_low <- du
du <- du +minespace
...
du<-max(as.matrix(unlist(size_all))) + max(as.matrix(unlist(size_low))) + minespace
d_low <- du
lans <-lapply(1:length(p_low$s),function(i){
spheres3d(p_low$f[i],du,0,radius=p_low$s[i],color=rgb((p_low$r)$col[[i]][1],(p_low$r)$col[[i]][2],(p_low$r)$col[[i]][3],alpha=1))
text3d(p_low$f[i],du + max(as.matrix(unlist(size_low))) +5,0,length(size_all) + i)
if(ad==0){text3d(p_low$f[i],(du - max(as.matrix(unlist(size_low))) -5),0,paste(unique(p_low$q1_all_a[[i]]),collapse=""))}
})
...
du <- d_mid1
l<-10
lans <- lapply( 1:length(size_mid1), function(i){
spheres3d(m+p_mid1$f[i],du,l,radius=p_mid1$s[i],color=rgb((p_mid1$r)$col[[i]][1],(p_mid1$r)$col[[i]][2],(p_mid1$r)$col[[i]][3],alpha=1))
text3d(m+p_mid1$f[i],du+max(as.matrix(unlist(size_mid1)))+5,l,length(size_all) + length(size_low) +i)
if(ad==0){text3d(m+p_mid1$f[i],(du - max(as.matrix(unlist(size_mid1))) -5),l,paste(unique(p_mid1$q1_all_a[[i]]),collapse=""))}
})
...
spheres3d(m+p_mid2$f[i],du,l,radius=p_mid2$s[i],color=rgb((p_mid2$r)$col[[i]][1],(p_mid2$r)$col[[i]][2],(p_mid2$r)$col[[i]][3],alpha=1))
text3d(m+p_mid2$f[i],du+max(as.matrix(unlist(size_mid2)))+ 5,l,length(size_all) + length(size_low) +length(size_mid1) + i)
if(ad==0){text3d(m+p_mid2$f[i],(du - max(as.matrix(unlist(size_mid2))) -5),l,paste(unique(p_mid2$q1_all_a[[i]]),collapse=""))}
})
...
spheres3d(m+p_high$f[i],du,l,radius=p_high$s[i],color=rgb((p_high$r)$col[[i]][1],(p_high$r)$col[[i]][2],(p_high$r)$col[[i]][3],alpha=1))
text3d(m+p_high$f[i],du+max(as.matrix(unlist(size_high)))+ 5,l,length(size_all) + length(size_low) +length(size_mid1) +length(size_mid2) +i)
if(ad==0){text3d(m+p_high$f[i],(du - max(as.matrix(unlist(size_high))) -5),l,paste(unique(p_high$q1_all_a[[i]]),collapse=""))}
})
...
segments3d(c((((i-1)/100)*(max(m+max(p_high$f),max(p$f)))),((i/100)*(max(m+max(p_high$f),max(p$f))))),c(u,u),c(l,l),col=matlab.like2(100)[i], lwd = 20)
})

text3d(-1-100,u,l,"Mean Deviation")
if(mean_value_m1 == "N"){
text3d(-40,u,l, round(qmin/length(select),2))
text3d(max(m+max(p_high$f),max(p$f))+40,u,l,round(qmax/length(select),2))
}
else{
...
if(length(p$q1_all)!=0){for(i in 1: length(size_all)){
	e_1 <- paste(p$q1_all[[i]][1],paste(ddd[p$q1_all[[i]][1],ni],")"),sep=" (")
	if(length(p$q1_all[[i]])>1){
	for(j in 2:length(p$q1_all[[i]])){
		
		e_1 <- paste(paste(e_1,p$q1_all[[i]][j],sep=","),paste(ddd[p$q1_all[[i]][j],ni],")"),sep=" (")
	}
	}
e[[i]]<- paste(i, e_1, sep = ":" )
}}
...
if(length(p_low$q1_all)!=0){for(i in 1: length(size_low)){
	e_1 <- paste(p_low$q1_all[[i]][1],paste(ddd[p_low$q1_all[[i]][1],ni],")"),sep="(")
	if(length(p_low$q1_all[[i]])>1){
	for(j in 2:length(p_low$q1_all[[i]])){
		
		e_1 <- paste(paste(e_1,p_low$q1_all[[i]][j],sep=","),paste(ddd[p_low$q1_all[[i]][j],ni],")"),sep="(")
	}
	}
...
if(length(p_mid1$q1_all)!=0){for(i in 1: length(size_mid1)){
	e_1 <- paste(p_mid1$q1_all[[i]][1],paste(ddd[p_mid1$q1_all[[i]][1],ni],")"),sep="(")
	if(length(p_mid1$q1_all[[i]])>1){
	for(j in 2:length(p_mid1$q1_all[[i]])){
		
		e_1 <-paste(paste(e_1,p_mid1$q1_all[[i]][j],sep=","),paste(ddd[p_mid1$q1_all[[i]][j],ni],")"),sep="(")
	}
	}
e[[i+d]]<- paste(i+d, e_1, sep = ":" )
}
...
if(length(p_mid2$q1_all)!=0){for(i in 1: length(size_mid2)){
	e_1 <- paste(p_mid2$q1_all[[i]][1],paste(ddd[p_mid2$q1_all[[i]][1],ni],")"),sep="(")
	if(length(p_mid2$q1_all[[i]])>1){
	for(j in 2:length(p_mid2$q1_all[[i]])){
		
		e_1 <- paste(paste(e_1,p_mid2$q1_all[[i]][j],sep=","),paste(ddd[p_mid2$q1_all[[i]][j],ni],")"),sep="(")
	}
	}
e[[i+d]]<-paste(i+d, e_1, sep = ":" )
}}

d <- length(size_all)+length(size_low)+length(size_mid1)+length(size_mid2)
if(length(p_high$q1_all)!=0){for(i in 1: length(size_high)){
	e_1 <- paste(p_high$q1_all[[i]][1],paste(ddd[p_high$q1_all[[i]][1],ni],")"),sep="(")
	if(length(p_high$q1_all[[i]])>1){
	for(j in 2:length(p_high$q1_all[[i]])){
		
		e_1 <- paste(paste(e_1,p_high$q1_all[[i]][j],sep=","),paste(ddd[p_high$q1_all[[i]][j],ni],")"),sep="(")
	}
	}
e[[i+d]]<-paste(i+d, e_1, sep = ":" )
}}
write.table(e,file=paste(filename,"description.txt",sep="_"),quote=FALSE,sep="\n",row.names=FALSE,col.names=FALSE)
junk <- list(low_map=p_low$q1_all,mid1_map=p_mid1$q1_all,mid2_map=p_mid2$q1_all,high_map=p_high$q1_all,all=p$q1_all)
