...
w <- abs(A[A!=0,])
w2 <- sign(w)}
select2 <- names(w2[w2[]>0]) #we just don't want those which are different
select<-names(w[w[]>alpha]) # we will only consider those which are different than a certain threshold
